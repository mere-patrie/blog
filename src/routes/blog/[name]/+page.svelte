...
<div class="max-w-5xl mx-auto w-full rounded-lg my-5 bg-white border border-gray-200 overflow-hidden">
    <img src="{post.banner}" alt="Post banner" class="blogCoverImg" data-flip-id="post-cover-img-{post.title.split(" ").join("-")}">
    <h1 class="m-5 blogTitle" data-flip-id="post-title-{post.title.split(" ").join("-")}">{post.title}</h1>
    {#if post }
